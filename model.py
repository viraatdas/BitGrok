...
def apply_rules(rules):
    def _apply_rules(path, value):
        ...
        path_list = [str(i.key).split("/")
                     for i in path if isinstance(i, jax.tree_util.DictKey)]
...
def init_layer_memories(
    ...
    return [
        KVMemory(
            k=jnp.zeros((batch_size, sequence_len,
...
class Router(hk.Module):
    ...
    def _compute_routing_prob(
        ...
        routing_logits = self._router_weights(
            inputs, num_experts, sharding=P("data"))
...
class MoELayer(hk.Module):
    ...
    def _inference_call(self, inputs: jax.Array, padding_mask: Optional[jax.Array] = None):
        ...
        tmp = jnp.reshape(
            inputs, (inputs.shape[0] * inputs.shape[1], inputs.shape[2]))
        broad_inputs = jnp.tile(
            tmp[:, jnp.newaxis, :], (1, self.router.num_selected_experts, 1))
        broad_inputs = jnp.reshape(
            broad_inputs, (broad_inputs.shape[0] *
                           broad_inputs.shape[1], broad_inputs.shape[2])
        )
        ...
        def moe_slow_matmul1(input, weight, scales, index, prob):
            ...
            output = jnp.einsum(
                "bm,bmn->mn", one_hot_indices, all_expert_output)
            return output
...
class TransformerConfig:
    ...
    def make(self, mesh=None) -> "Transformer":
        data_axis = tuple(self.data_axis) if isinstance(
            self.data_axis, list) else self.data_axis
        model_axis = (
            tuple(self.model_axis) if isinstance(
                self.model_axis, list) else self.model_axis
        )
...
class MultiHeadAttention(hk.Module):
    ...
    def __call__(
        ...
        key_heads = rotate(key_heads, seq_dim=1, offset=(
            kv_memory.step if kv_memory else 0))
        query_heads = rotate(query_heads, seq_dim=1, offset=(
            kv_memory.step if kv_memory else 0))
        ...
        if kv_memory:
            if mesh is not None:
                ...
                key_heads = update_into_shmap(
                    kv_memory.k, kv_memory.step, key_heads)
...
class MHABlock(hk.Module):
    ...
    def __call__(
        ...
        attn_output = attn_block(
            inputs, side_input, side_input, mask, layer_memory)
        ...
        *,
        ...
        if self.model.shard_activations:
            out = with_sharding_constraint(
                out, P("data", None, self.model.model_axis))
        else:
