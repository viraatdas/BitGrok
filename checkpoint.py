...
# The model checkpoint stored is in 8 bit
...
def load_tensors(shaped_arrays, directory, mesh_config, tensor_indices=None):
    ...
    for i, t in iterator:
        if (i % num_replicas) == ((jax.process_index() // data_model_shards) % num_replicas):
            ...
            fs.append(
                pool.submit(fast_unpickle, os.path.join(
                    directory, f"tensor{i:05d}_{idx:03d}"))
            )
            num_tensors += 1
        else:
...
def replace_with_load_state(
    ...
    flatten_init, structure_init = jax.tree_util.tree_flatten_with_path(
        init_state)
    load_map = {path_tuple_to_string(
        path): tensor for path, tensor in flatten_load}
...
def restore(
    ...
    ckpt_shapes_with_path, structure = jax.tree_util.tree_flatten_with_path(
        ckpt_shapes)
