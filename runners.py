...
def top_p_filter(logits: jax.Array, top_p: jax.Array) -> jax.Array:
    ...
    threshold_idx = jnp.argmax(jnp.cumsum(
        sorted_probs, -1) >= 1 - top_p, axis=-1)
...
def sample_token(
    ...
    logits = lm_outputs.logits / \
        settings.temperature.astype(lm_outputs.logits.dtype)
...
class ModelRunner:
    ...
    def initialize(
        ...
        self.mesh = make_mesh(self.local_mesh_config,
                              self.between_hosts_config)
    ...
    def quantize_to_1bit(self, state: TrainingState) -> TrainingState:
        """
        Converts the model's weights from the checkpoint file of 8-bit quantized format to 1-bit.

        Args:
            state: TrainingState instance containing the model's parameters.

        Returns:
            A new TrainingState instance with 1-bit quantized weights.
        """
        def quantize_fn(weight):
            # Assuming `weight` is a QuantizedWeight8bit instance.
            mean_abs = jnp.mean(jnp.abs(weight.weight))
            new_scale = mean_abs * weight.scales
            quantized_weight = jnp.sign(weight.weight)
            return QuantizedWeight8bit(weight=quantized_weight, scales=new_scale)

        # Apply quantization function to all QuantizedWeight8bit instances in the model's parameters.
        new_params = jax.tree_map(lambda x: quantize_fn(x) if isinstance(
            x, QuantizedWeight8bit) else x, state.params)
    ...
    def load_or_init(
        ...
        if not self.checkpoint_path or not from_checkpoint:
            ...
            state = self.quantize_to_1bit(state)
    ...
    def initialize(self):
        ...
        def hk_prefill_memory(
            ...
            slice = slice._replace(
                layers=[l._replace(step=jnp.array([length]))
